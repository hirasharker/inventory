...
class Sales_Order_Model extends CI_Model {
    ...
    public function get_all_sales_orders_mysql(){
        $this->db->select('tbl_sales_order.sales_order_id, tbl_sales_order.customer_id, tbl_sales_order.customer_name, tbl_sales_order.customer_type, tbl_sales_order.dealer_name, tbl_sales_order.user_id, tbl_sales_order.user_name
            , tbl_sales_order.sales_order_date, STRING_AGG(CONCAT_WS( "|" , DDD_1, tbl_sales_order_detail.item_name),"|") AS item_name, (sum(tbl_sales_order_detail.sales_order_price * tbl_sales_order_detail.quantity-tbl_sales_order_detail.individual_discount))*(1-.01*tbl_sales_order.overall_discount) as total_price'); 
